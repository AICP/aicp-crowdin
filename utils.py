...
def get_username(args):
    ...
    if (args.gerrit or args.download or args.unzip) and username is None:
        ...
        msg, code = run_subprocess(
            ["git", "config", "--get", "review.gerrit.aicp-rom.com.username"],
            silent=True,
        )
        has_username = False
        if code == 0:
...
def get_xml_files(base_path, default_branch):
    xml_default = load_xml(x=f"{_DIR}/config/{default_branch}_crowdin.xml")
    ...
    xml_snippet = load_xml(x=f"{base_path}/platform_manifest/snippets/aicp.xml")
    if xml_snippet is not None:
        xml_files = (xml_default, xml_snippet, xml_extra)
    else:
...
def get_config_dict(config, default_branch):
    ...
    # Disable non-AOSP languages as we usually do not use those
...
def get_gerrit_base_cmd(username):
    cmd = ["ssh", "-p", "29418", f"{username}@gerrit.aicp-rom.com", "gerrit"]
    return cmd
